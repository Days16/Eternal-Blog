...
type LoginFormProps = { userCount: number }

export function LoginForm({ userCount }: LoginFormProps) {
  return (
    <Suspense>
      <LoginContent userCount={userCount} />
    </Suspense>
  )
}

function LoginContent({ userCount }: LoginFormProps) {
    ...
    <div style={{
      ...
      <div className="auth-decor-panel" style={{
        ...
        <div style={{ textAlign: 'center', position: 'relative', zIndex: 1 }}>
          ...
          {userCount > 0 && (
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--rune)', letterSpacing: 1, marginTop: 18, textTransform: 'uppercase' }}>
              {userCount.toLocaleString('es-ES')} investigadores de Alexandra ya exploran el archivo
            </p>
          )}
        </div>
      </div>
