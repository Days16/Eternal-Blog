      ...
      <div style={{ display: 'grid', gap: 12 }}>
        {users.map(user => (
          <div key={user.id} style={{ display: 'grid', gridTemplateColumns: '1fr 220px 220px', gap: 16, alignItems: 'center', background: 'var(--bg-card)', border: '1px solid var(--border-soft)', borderRadius: 'var(--r-lg)', padding: 16 }}>
            ...
            <form action={updateUserRoleAction} style={{ display: 'flex', gap: 8 }}><input type="hidden" name="id" value={user.id} /><select name="role" defaultValue={user.role} style={inputStyle}>{ROLES.map(role => <option key={role}>{role}</option>)}</select><Btn size="sm" variant="ghost">Rol</Btn></form>
